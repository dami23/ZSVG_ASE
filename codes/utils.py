    ...
    def add_value(self, val: Dict[str, torch.tensor]):
        for k in self.keys:
            ...
            if hasattr(value, 'numel') and value.numel() > 1:
                value = value.mean()
            self.smooth_vals[k].add_value(value)
...
class Learner:
    uid: str
    cfg: Dict
    data: DataWrap # DataWrap_RefCOCO
    mdl: nn.Module
    loss_fn: nn.Module
    eval_fn: nn.Module
    opt_fn: Callable
    ...
    def init_log_dirs(self):
        ...
        """
        # Saves the text logs
        self.txt_log_file = Path(self.data.path) / 'txt_logs' / self.cfg['ds_to_use'] / f'{self.uid}.txt'
        ...
        self.model_file = Path(self.data.path) / 'models' / self.cfg['ds_to_use'] / f'{self.uid}.pth'
    ...
    def validate(self, db: Optional[DataLoader] = None,
        ...
        with torch.no_grad():
            ...
            for batch in progress_bar(db, parent=mb):
                ...
                out = self.mdl(batch)
                out_loss = self.loss_fn(out, batch, None)
    ...
    def train_epoch(self, mb) -> List[torch.tensor]:
        ...
        for batch_id, batch in enumerate(progress_bar(self.data.train_dl, parent=mb)):
            ...
            comment_to_print = f'LossB {loss.item(): .4f} | SmLossB {trn_loss.smooth1.item(): .4f} | AccB {trn_acc.smooth1.item(): .4f}'
        ...
        try:
            ...
            for epoch in mb:
                ...
                mb_write = [str(stat) if isinstance(stat, int)
                            else f'{stat.item():.4f}' for stat in to_write]
