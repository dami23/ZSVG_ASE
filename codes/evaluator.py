...
class Evaluator(nn.Module):
    ...
    def forward(self, out: Dict[str, torch.tensor], inp: Dict[str, torch.tensor]) -> Dict[str, torch.tensor]:     
        annot = inp['annot']
        att_box = out['att_out']
        reg_box = out['bbx_out']
        ...
        best_possible_result, _ = self.get_eval_result(actual_bbox, annot, expected_best_ids)
            
        msk = None
        actual_result, pred_boxes = self.get_eval_result(actual_bbox, annot, att_box_best_ids, msk)
        ...
        reshaped_boxes = x1y1x2y2_to_y1x1y2x2(reshape((pred_boxes + 1)/2, (inp['img_size'])))
